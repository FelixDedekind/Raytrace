...
void initialize_beam(Beam *beam, int theta, int phi) {
    ...
    double theta_step = (theta_max - theta_min) / theta_res;
    double phi_step = (phi_max - phi_min) / phi_res;

    // Calculate the actual theta and phi values
    double theta_val = theta_min + theta * theta_step;
    double phi_val = phi_min + phi * phi_step;
